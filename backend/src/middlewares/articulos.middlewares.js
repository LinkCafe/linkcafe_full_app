...
export const middlewaresShowArticles = [
    check('nombre','nombre, Campo obligatorio').not().isEmpty().isLength({ min: 3, max: 100 }).isString().optional(),
    check('enlace','enlace Obligatorio').not().isEmpty().isLength({ max: 225 }).optional(),
    check('autor','autor obligatorio').not().isEmpty().isLength({ min: 3, max: 100 }).optional(),
];
